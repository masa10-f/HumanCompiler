...
function typeNote(editor: HTMLElement, text: string) {
  fireEvent.paste(editor, {
    clipboardData: {
      getData: (type: string) => (type === "text/plain" ? text : ""),
    },
...
function savedBlocks() {
  return (changed.mock.calls.at(-1)![0] as DailyPlanDocumentV1).blocks;
}

it.each(["1100-1200 論文", "1100-1200 調査", "11:00-12:00 研究 読む"])(
  "links a fixed line to a task predicted from %s",
  async (text) => {
    render(<Notebook />);
    const editor = await screen.findByRole("textbox", { name: "日次ノート" });
    typeNote(editor, text);
    const suggestions = await screen.findByRole("listbox", {
      name: "固定予定に紐づけるタスク",
    });
    const candidate = within(suggestions).getByRole("option", {
      name: /論文を読む/,
    });
    expect(candidate).toHaveTextContent("研究 / 調査");
    expect(
      within(suggestions).getByRole("option", {
        name: "タスクに紐づけずに追加",
      }),
    ...
    fireEvent.keyDown(editor, { key: "Enter" });
    expect(await screen.findByText("固定予定")).toBeInTheDocument();
...
it("links a clicked fixed line candidate", async () => {
  render(<Notebook />);
  const editor = await screen.findByRole("textbox", { name: "日次ノート" });
  typeNote(editor, "0900-1000 論文");
  fireEvent.click(await screen.findByRole("option", { name: /論文を読む/ }));
  expect(await screen.findByText("固定予定")).toBeInTheDocument();
  expect(savedBlocks()[0]).toMatchObject({
    type: "timed_line",
    title: "論文を読む",
...
it("keeps Enter adding an unlinked fixed line while candidates are shown", async () => {
  render(<Notebook />);
  const editor = await screen.findByRole("textbox", { name: "日次ノート" });
  typeNote(editor, "1100-1200 研究");
  await screen.findByRole("listbox", { name: "固定予定に紐づけるタスク" });
  fireEvent.keyDown(editor, { key: "ArrowDown" });
  fireEvent.keyDown(editor, { key: "ArrowUp" });
  fireEvent.keyDown(editor, { key: "Enter" });
  expect(await screen.findByText("固定予定")).toBeInTheDocument();
  expect(savedBlocks()[0]).toMatchObject({ type: "timed_line", title: "研究" });
  expect(savedBlocks()[0]).not.toHaveProperty("task_ref");
});

it("lets ArrowUp leave a fixed line when no candidate is chosen", async () => {
  render(
    <Notebook
      initial={{
        schema_version: 1,
        blocks: [
          ...
          { id: "draft", type: "text", text: "1100-1200 論文" },
        ],
      }}
    />,
  );
  ...
  fireEvent(document, new Event("selectionchange"));
  await screen.findByRole("listbox", { name: "固定予定に紐づけるタスク" });
  // fireEvent returns false when the editor keeps the key for the candidates.
  expect(fireEvent.keyDown(editor, { key: "ArrowDown" })).toBe(false);
  expect(screen.getByRole("option", { name: /論文を読む/ })).toHaveAttribute(
    "aria-selected",
    "true",
  );
...
it("closes fixed line candidates with Escape and still adds the line", async () => {
  render(<Notebook />);
  const editor = await screen.findByRole("textbox", { name: "日次ノート" });
  typeNote(editor, "1100-1200 論文");
  await screen.findByRole("listbox", { name: "固定予定に紐づけるタスク" });
  fireEvent.keyDown(editor, { key: "Escape" });
  expect(screen.queryByRole("listbox")).not.toBeInTheDocument();
  expect(editor).toHaveTextContent("1100-1200 論文");
  fireEvent.keyDown(editor, { key: "Enter" });
  expect(await screen.findByText("固定予定")).toBeInTheDocument();
  expect(savedBlocks()[0]).toMatchObject({ type: "timed_line", title: "論文" });
  expect(savedBlocks()[0]).not.toHaveProperty("task_ref");
});

it.each(["1100-1200 会議", "/break 1200-1300 研究", "1100-1200"])(
  "does not suggest tasks for %s",
  async (text) => {
    ...
    expect(screen.queryByRole("listbox")).not.toBeInTheDocument();
    expect(search).toHaveBeenCalledTimes(text === "1100-1200 会議" ? 1 : 0);
  },
);
