...
def create_test_project_with_nested_data(session: Session, user_id, num_goals=5, num_tasks_per_goal=10, num_logs_per_task=3):
...
class TestProjectDeletionPerformance:
    ...
    def test_delete_project_large_dataset(self, test_session, test_user):
        ...
        assert result is True
        assert deletion_time < 5.0  # Should complete in under 5 seconds even for large dataset
        print(f"Large dataset deletion time: {deletion_time:.4f} seconds")
        
        # Verify all data was deleted
        from sqlmodel import select
        remaining_goals = test_session.exec(select(Goal).where(Goal.project_id == project.id)).all()
