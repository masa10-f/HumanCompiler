...
from taskagent_api.models import Goal, Project, Task, TaskStatus
...
def test_openai_client_unavailable():
    ...
    with patch(
        "taskagent_api.ai.openai_client.settings.openai_api_key", "your_openai_api_key"
    ):
        client = OpenAIClient()
...
async def test_context_collector():
    ...
    with patch.object(collector.project_service, "get_projects", return_value=[]):
        with patch.object(
            ...
            with patch.object(
                collector.task_service, "get_tasks_by_goal", return_value=[]
            ):
                mock_session = Mock()
...
async def test_weekly_plan_service():
    ...
    with patch.object(
        ...
        with patch.object(
            service.openai_client, "generate_weekly_plan"
        ) as mock_generate:
