...
DROP POLICY IF EXISTS slot_templates_user_policy ON public.slot_templates;
...
COMMENT ON TABLE public.slot_templates IS 'Stores day-of-week slot presets for scheduling. Each user can have multiple templates per day with one marked as default.';
COMMENT ON COLUMN public.slot_templates.day_of_week IS 'Day of week following ISO 8601: 0=Monday, 6=Sunday';
COMMENT ON COLUMN public.slot_templates.slots_json IS 'Array of TimeSlot objects with start, end, kind, and optional capacity_hours/assigned_project_id';
COMMENT ON COLUMN public.slot_templates.is_default IS 'Whether this template is automatically applied for the day of week';
