...
class BackupRecord:
    ...
    tables_backed_up: Dict[str, int] = None  # table_name -> record_count
    ...
    async def create_backup(self, backup_type: str = "manual") -> BackupRecord:
        ...
        try:
            ...
            with open(backup_path, 'r') as f:
                ...
                tables_backed_up = backup_data.get("metadata", {}).get("total_records", {})
            ...
            if self.config.alert_on_failure:
                await self._send_alert(f"Backup Failed", f"{backup_type.capitalize()} backup failed: {e}")
...
This is an automated alert from the TaskAgent backup system.
    ...
    def cleanup_old_backups(self):
        ...
        for record in sorted_backups:
            ...
            if age.total_seconds() < 3600:  # Less than 1 hour old
                ...
                keep_backup = True
                hourly_count += 1
            elif age.days <= 30 and record.backup_type == "daily" and daily_count < self.config.keep_daily_backups:
                keep_backup = True
                daily_count += 1
            elif age.days <= 90 and record.backup_type == "weekly" and weekly_count < self.config.keep_weekly_backups:
                keep_backup = True
                weekly_count += 1
            elif record.backup_type == "monthly" and monthly_count < self.config.keep_monthly_backups:
                keep_backup = True
                monthly_count += 1
            ...
            if keep_backup:
                ...
                try:
                    ...
                    logger.warning(f"Failed to remove backup file {record.backup_path}: {e}")
    ...
    def start(self):
        ...
        self.scheduler.add_job(
            self.create_backup,
            trigger=CronTrigger(day_of_week=self.config.weekly_backup_day, hour=hour, minute=minute),
            args=["weekly"],
            id="weekly_backup",
            max_instances=1,
            coalesce=True
        ...
        logger.info(f"   🕐 Interval backups: every {self.config.backup_interval_hours} hours")
        logger.info(f"   📅 Daily backups: {self.config.daily_backup_time}")
        logger.info(f"   📈 Weekly backups: every {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][self.config.weekly_backup_day]}")
        logger.info(f"   🗓️  Monthly backups: 1st of each month")
    ...
    async def health_check(self):
        ...
        try:
            ...
            if self.backup_history:
                ...
                recent_backup_ok = (
                    time_since_last.total_seconds() < self.config.backup_interval_hours * 3600 * 2
            ...
            if not all_healthy:
                ...
                await self._send_alert(
                    ...
                    f"Health check issues detected:\n" + "\n".join(f"- {issue}" for issue in issues)
                )
            
            logger.debug(f"Health check: DB={db_healthy}, Dir={dir_exists}, Recent={recent_backup_ok}, Disk={disk_space_ok}")
            
        except Exception as e:
            logger.error(f"Health check failed: {e}")
            await self._send_alert("Health Check Error", f"Health check encountered an error: {e}")
    ...
    def get_backup_status(self) -> Dict[str, Any]:
        ...
        latest_backup = max(self.backup_history, key=lambda x: x.timestamp)
        successful_backups = [b for b in self.backup_history if b.status == BackupStatus.SUCCESS]
        failed_backups = [b for b in self.backup_history if b.status == BackupStatus.FAILED]
        
        return {
            "status": "healthy" if latest_backup.status == BackupStatus.SUCCESS else "unhealthy",
            "last_backup": {
...
def init_backup_scheduler(config: Optional[BackupConfig] = None) -> BackupScheduler:
