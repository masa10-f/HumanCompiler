...
class BackupRecordResponse(BaseModel):
    ...
    tables_backed_up: dict[str, int] | None = None
...
class BackupOperationResponse(BaseModel):
    ...
    success: bool
    message: str
    backup_record: BackupRecordResponse | None = None
    ...
    try:
        ...
        if status_filter:
            try:
                ...
                history = [
                    record for record in history if record.status == filter_status
                ]
            except ValueError:
                raise HTTPException(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get backup history: {str(e)}"
        )
...
async def create_backup(
    ...
    try:
        ...
        return BackupOperationResponse(
            success=False, message=f"Failed to create backup: {str(e)}"
        )


@router.post("/restore", response_model=BackupOperationResponse)
async def restore_backup(
    backup_filename: str, current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        if not backup_path.suffix == ".json":
            raise HTTPException(
                status_code=400, detail="Only JSON backup files are supported"
            )
        ...
        return BackupOperationResponse(
            success=False, message=f"Failed to restore backup: {str(e)}"
        )
...
async def start_scheduler(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if scheduler.is_running:
            return BackupOperationResponse(
                success=True, message="Backup scheduler is already running"
            )

        scheduler.start()
        return BackupOperationResponse(
            success=True, message="Backup scheduler started successfully"
        )

    except Exception as e:
        logger.error(f"Failed to start scheduler: {e}")
        return BackupOperationResponse(
            success=False, message=f"Failed to start scheduler: {str(e)}"
        )
...
async def stop_scheduler(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        if not scheduler.is_running:
            return BackupOperationResponse(
                success=True, message="Backup scheduler is not running"
            )

        scheduler.stop()
        return BackupOperationResponse(
            success=True, message="Backup scheduler stopped successfully"
        )

    except Exception as e:
        logger.error(f"Failed to stop scheduler: {e}")
        return BackupOperationResponse(
            success=False, message=f"Failed to stop scheduler: {str(e)}"
        )


@router.post("/scheduler/configure", response_model=BackupOperationResponse)
async def configure_scheduler(
    config_request: BackupConfigRequest, current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        return BackupOperationResponse(
            success=True, message="Backup scheduler configuration updated successfully"
        )

    except Exception as e:
        logger.error(f"Failed to configure scheduler: {e}")
        return BackupOperationResponse(
            success=False, message=f"Failed to configure scheduler: {str(e)}"
        )
...
async def cleanup_old_backups(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return BackupOperationResponse(
            success=False, message=f"Failed to cleanup backups: {str(e)}"
        )
