...
async def generate_weekly_plan(
    ...
    """
    try:
        logger.info(f"Generating weekly plan for user {user_id} starting {request.week_start_date}")
        ...
        logger.info(f"Weekly plan generated: {len(plan_response.task_plans)} tasks planned")
    ...
    try:
        ...
        for task in all_tasks:
            ...
            if goal:
                ...
                if project:
                    project_hours[project.title] = project_hours.get(project.title, 0) + task.estimate_hours
        
        # Generate recommendations
        recommendations = []
        
        if total_hours > 40:
            recommendations.append(f"Workload is {total_hours:.1f} hours - consider prioritizing or deferring some tasks")
        
        if len(overdue_tasks) > 0:
            recommendations.append(f"{len(overdue_tasks)} overdue tasks require immediate attention")
        
        if len(urgent_tasks) > 0:
            recommendations.append(f"{len(urgent_tasks)} tasks are due within 3 days")
        
        if len(project_hours) > 3:
            recommendations.append("Consider focusing on fewer projects to maintain momentum")
    ...
    try:
        ...
        for task in tasks:
            ...
            task_scores.append({
                ...
                "suggested_priority": min(5, max(1, 6 - (score // 15))),  # Convert to 1-5 scale
                "reasoning": reasons
            })
